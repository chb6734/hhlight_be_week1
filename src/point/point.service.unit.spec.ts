  ...
  describe('유닛 테스트', () => {
    describe('getPoint', () => {
      ...
      it('사용자의 ID 값을 양의 정수 형태로 주지 않을 경우 실패', async () => {
        await expect(pointService.getPoint(0)).rejects.toThrow(
          '올바르지 않은 ID 값 입니다.',
        );
        await expect(pointService.getPoint(-1)).rejects.toThrow(
          '올바르지 않은 ID 값 입니다.',
        );
        await expect(pointService.getPoint(1.5)).rejects.toThrow(
          '올바르지 않은 ID 값 입니다.',
        );
        await expect(pointService.getPoint(NaN)).rejects.toThrow(
          '올바르지 않은 ID 값 입니다.',
        );
        await expect(pointService.getPoint(Infinity)).rejects.toThrow(
          '올바르지 않은 ID 값 입니다.',
        );
        await expect(pointService.getPoint(-Infinity)).rejects.toThrow(
          '올바르지 않은 ID 값 입니다.',
    ...
    describe('getHistories', () => {
      ...
      it('사용자1이 포인트를 충전/사용한 내역을 확인하면 2건의 내역이 있음', async () => {
        ...
        (historyDbStub.selectAllByUserId as jest.Mock).mockResolvedValue(
          mockHistories,
        );
       ...
       */
      it('사용자의 ID 값을 양의 정수 형태로 주지 않을 경우 실패', async () => {
        await expect(pointService.getHistory(0)).rejects.toThrow(
          '올바르지 않은 ID 값 입니다.',
        );
        await expect(pointService.getHistory(-1)).rejects.toThrow(
          '올바르지 않은 ID 값 입니다.',
        );
        await expect(pointService.getHistory(1.5)).rejects.toThrow(
          '올바르지 않은 ID 값 입니다.',
        );
        await expect(pointService.getHistory(NaN)).rejects.toThrow(
          '올바르지 않은 ID 값 입니다.',
        );
        await expect(pointService.getHistory(Infinity)).rejects.toThrow(
          '올바르지 않은 ID 값 입니다.',
        );
    ...
    describe('chargePoint', () => {
      ...
      it('100포인트가 있는 사용자1에게 50포인트를 추가하면 총 150포인트가 됨', async () => {
        ...
        (userDbStub.selectById as jest.Mock).mockResolvedValue(mockUserPoint);
        (userDbStub.insertOrUpdate as jest.Mock).mockResolvedValue(
          mockUserUpdatedPoint,
        );
       ...
       */
      it('사용자의 ID 값을 양의 정수 형태로 주지 않을 경우 실패', async () => {
        await expect(pointService.chargePoint(0, 100)).rejects.toThrow(
          '올바르지 않은 ID 값 입니다.',
        );
        await expect(pointService.chargePoint(-1, 100)).rejects.toThrow(
          '올바르지 않은 ID 값 입니다.',
        );
        await expect(pointService.chargePoint(1.5, 100)).rejects.toThrow(
          '올바르지 않은 ID 값 입니다.',
        );
        await expect(pointService.chargePoint(NaN, 100)).rejects.toThrow(
          '올바르지 않은 ID 값 입니다.',
        );
        await expect(pointService.chargePoint(Infinity, 100)).rejects.toThrow(
          '올바르지 않은 ID 값 입니다.',
        );
    ...
    describe('usePoint', () => {
      ...
      it('사용자의 ID 값을 양의 정수 형태로 주지 않을 경우 실패', async () => {
        ...
        await expect(pointService.usePoint(1.5, 100)).rejects.toThrow(
          '올바르지 않은 ID 값 입니다.',
        );
        await expect(pointService.usePoint(NaN, 100)).rejects.toThrow(
          '올바르지 않은 ID 값 입니다.',
        );
        await expect(pointService.usePoint(Infinity, 100)).rejects.toThrow(
          '올바르지 않은 ID 값 입니다.',
        );
      ...
      it('음수, 0, 실수, NaN, Infinity 값으로 포인트를 사용할 경우 실패', async () => {
        ...
        await expect(pointService.usePoint(1, 1.5)).rejects.toThrow(
          '사용할 포인트는 정수여야 하며 0보다 커야합니다.',
        );
        await expect(pointService.usePoint(1, NaN)).rejects.toThrow(
          '사용할 포인트는 정수여야 하며 0보다 커야합니다.',
        );
      ...
      it('100포인트가 있는 사용자1이 101포인트를 사용할 경우 실패', async () => {
        ...
        await expect(pointService.usePoint(1, 101)).rejects.toThrow(
          '포인트가 부족합니다.',
        );
